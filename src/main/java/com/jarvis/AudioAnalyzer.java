...
public class AudioAnalyzer {
     ...
     * @return FadeoutInfo containing the fadeout timestamp, or null if no fadeout
     *         detected or file is unreadable
     */
     ...
     * 
     * @param filePath Absolute path to the audio file
     * @return FadeoutInfo containing the fadeout timestamp, or null if analysis
     *         fails
     */
                ...
                if (tempWavPath != null) {
                    ...
                    if (tempFile.exists()) {
                        if (tempFile.delete()) {
                            ...
                            System.err
                                    .println("[-] AudioAnalyzer: Could not delete temporary WAV file: " + tempWavPath);
                        }
                    }
                }
            }
        ...
        try {
            ...
            if (exitCode == 0) {
                ...
                if (tempFile.exists() && tempFile.length() > 0) {
                    if (App.DEBUG_MODE) {
                        System.out.println(
                                "[DEBUG] AudioAnalyzer: FFmpeg conversion succeeded (" + tempFile.length() + " bytes)");
                    }
                    return tempPath;
                } else {
    ...
    private static FadeoutInfo performAnalysis(String filePath) throws Exception {
        ...
        // First, calculate average volume from the ENTIRE song (not just the fadeout
        // window)
        // This ensures we're comparing against the song's normal volume, not the
        // fadeout section
        List<Float> fullSongVolumeLevels = extractVolumeLevels(audioFile, audioFormat, 0, sampleRate);

        if (fullSongVolumeLevels.isEmpty()) {
            if (App.DEBUG_MODE) {
                System.out.println("[DEBUG] AudioAnalyzer: No volume data extracted from full song");
            }
            return null;
        }

        // Calculate average volume from the entire song
        float overallAverageVolume = 0;
        for (float level : fullSongVolumeLevels) {
            overallAverageVolume += level;
        }
        overallAverageVolume /= fullSongVolumeLevels.size();

        // Now extract volume levels from just the last 8 seconds to search for fadeout
        List<Float> fadeoutWindowVolumeLevels = extractVolumeLevels(audioFile, audioFormat, analysisStartSeconds,
                sampleRate);

        if (fadeoutWindowVolumeLevels.isEmpty()) {
            if (App.DEBUG_MODE) {
                System.out.println("[DEBUG] AudioAnalyzer: No volume data extracted from fadeout window");
            }
            return null;
        }
        ...
        if (App.DEBUG_MODE) {
            System.out.println("[DEBUG] AudioAnalyzer: Fadeout detected at " +
                    String.format("%.1f", fadeoutTimestamp) + "s (overall avg=" +
                    String.format("%.2f", overallAverageVolume) + ", threshold=" +
                    String.format("%.2f", volumeThreshold) + ")");
        }

        return new FadeoutInfo(filePath, fadeoutTimestamp, durationSeconds, overallAverageVolume);
    }
    ...
    /**
     * Finds the first point where volume drops below the threshold AND stays low
    ...
    private static int findFadeoutPoint(List<Float> volumeLevels, float volumeThreshold,
        ...
        for (int i = startSearchIndex; i < volumeLevels.size(); i++) {
            ...
            // Track consecutive low-volume samples
        ...
        return -1; // No sustained fadeout detected
    }
